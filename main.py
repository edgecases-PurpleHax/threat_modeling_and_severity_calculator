...
def parse_args():
    ...
    parser.add_argument("-t", "--threat-model", action="store_true", help="Use to perform a threat model")
    parser.add_argument("-s", "--severity",
                        action="store_true",
                        help="Use to perform a severity rating. "
                             "Use with -O/--OWASP, -S/--STRIDE, -C/--CVSS, or -D/--DREAD")
    parser.add_argument("-D", "--DREAD", action="store_true", help="Use with -s to perform DREAD Severity Rating")
    parser.add_argument("-S", "--STRIDE", action="store_true", help="Use with -s to perform STRIDE Severity Rating")
    parser.add_argument("-C", "--CVSS", action="store_true", help="Use with -s to perform CVSS Severity Rating")
    parser.add_argument("-O", "--OWASP", action="store_true", help="Use with -s to perform OWASP Severity Rating")
    ...
    if args.severity:
        ...
        while next_vulnerability.lower()[0] == "y":
            ...
            if args.CVSS:
                ...
                sys.exit()
            next_vulnerability = input("Would you like to Calculate another Vulnerability?")
    else:
        print("Use python main.py -h for help")
        sys.exit()