...
5: Critical"""
    ...
    def get_rating(self):
        ...
        try:
            ...
            self.reproducibility = int(
                input("What is the reproducibility rating? "))
            self.reproducbility_explanation = self.get_explanation()
            self.exploitability = int(
                input("What is the exploitability rating? "))
            self.exploitability_explanation = self.get_explanation()
            self.affectedusers = int(
                input("What is the Affected Users rating? "))
            self.affectedusers_explanation = self.get_explanation()
            self.discoverability = int(
                input(
                    "What is the discoverability rating? (Use 5 if not sure) ")
            )
            self.discoverability_explanation = self.get_explanation()
            rating = (self.damage + self.reproducibility + self.exploitability
    ...
    def get_rating(self):
        ...
        try:
            ...
            self.skill_level = int(
                input(
                    "What is the technical skill level of the threat agents? ")
            )
            self.skill_level_explanation = self.get_explanation()
            self.motive = int(
                input("How motivated is the group of threat agents? "))
            ...
            self.detection = int(
                input("How likely is the exploit to be detected? "))
            self.detection_explanation = self.get_explanation()
            self.likelihood = (self.skill_level + self.motive +
                               self.opportunity + self.size +
                               self.discoverability + self.exploitability +
                               self.awareness + self.detection) / 8
            ...
            self.availability = int(
                input(
                    "How much service loss could occur and how vital is it? "))
            ...
            self.technical = (self.confidentiality + self.availability +
                              self.integrity + self.accountability) / 4
            print("Step 4: Business Impact Factors")
            self.financial = int(
                input(
                    "How much financial damage would result from an exploit? ")
            )
            ...
            self.business = (self.financial + self.reputation +
                             self.compliance + self.privacy) / 4
...
5: Critical"""
    ...
    def get_rating(self):
        ...
        try:
            self.spoofing = int(
                input(
                    "What is the potential damage if identified spoofing occurs? "
                ))
            self.spoofing_explanation = self.get_explanation()
            self.tampering = int(
                input(
                    "What is the potential damage if identified tampering occurs? "
                ))
            ...
            self.information = int(
                input(
                    "What is the sensitivity of the information disclosed? "))
            ...
            self.eop = int(
                input("What is the risk of elevation of privileges? "))
            self.eop_explanation = self.get_explanation()
            rating = (self.spoofing + self.tampering + self.repudiation +
                      self.information + self.dos + self.eop) / 6
            if int(round(rating, 0)) == 1:
                return "This threat is rated as: Informational"
            if int(round(rating, 0)) == 2:
                return "This threat is rated as: Low"
...
5: Critical"""
    ...
    def get_rating(self):
        ...
        try:
            ...
            if availability == 3:
                availability = 5
            rating = (attack_vector + complexity + privs + user + scope +
                      confidentiality + integrity + availability) / 8
            if int(round(rating, 0)) == 1:
    ...
    def get_temporal(self):
        ...
        try:
            ...
            if base == "Critical":
                base_score = 5
            exploit = int(
                input("Enter level of exploit code maturity\r\n"
                      "2. No Exploit\r\n"
                      "4. Proof of concept\r\n"
                      "5. Funcitonal Exploit exists\r\n"))
            self.exploit = mapping["exploit"][exploit]
            remediation = int(
                input("Enter remediation level: \r\n"
                      "2. Official Fix\r\n"
                      "3. Temporary Fix\r\n"
                      "4. Workaround\r\n"
                      "5. Unavailable\r\n"))
            ...
            print(
                f"Something went wrong while creating the Temporal Score: {e}")
    ...
    def get_environmental(self):
        ...
        try:
            ...
            self.confidentiality_env = mapping["confidentiality"][
                confidentiality]
            if confidentiality == 3:
                confidentiality = 4
            integrity = int(
            ...
            print(
                f"Something went wrong while creating the Environmental Score: {e}"
            )
